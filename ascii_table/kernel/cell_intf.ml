...
module type Cell = sig @@ portable
  ...
  (** [is_empty t] returns true if every line is empty. Note that this means
      [is_empty (create [ [], s ]) <> String.is_empty s] when [s] is all '\n'. *)
  val is_empty : t -> bool
  ...
  (** [wrap_lines t ~width = lines] rewraps the lines of a cell to fit within [width].
      [wrap_lines] only adds new line breaks. *)
  val wrap_lines
    :  t
    -> width:int
